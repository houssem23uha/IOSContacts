...
class SecondTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cellIdentifier = "SecondContactCell"
        let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier) as! SecondTableViewCell
        
        let set = allContact().sorted {($0.phoneNumber ?? "") < ($1.phoneNumber ?? "")}
        
        cell.nameLabel?.text = set[indexPath.row].firstName! + " " + set[indexPath.row].secondName!
            
        cell.numberLabel?.text = set[indexPath.row].phoneNumber!
    ...
    */
    ...
    @IBAction func editingEndForSeque(returnSegue: UIStoryboardSegue){
        if let controller = returnSegue.source as? EditViewController{
